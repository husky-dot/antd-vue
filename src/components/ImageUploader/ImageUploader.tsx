...
export type ImagePartialFile = Partial<ImageFile> & {
  url: string;
};
...
export const ImageUploaderProps = {
   ...
   */
  value: {
    type: Array as PropType<ImagePartialFile[]>,
    default: [],
  },
  /**
   * 是否禁用
  ...
  setup(props, { emit, slots }) {
        ...
        return getImageFileInfo(file)
          .then((imageFileInfo: ImageFileInfo) => {
            ...
            if (props.resolutions) {
              const accord = props.resolutions.find((item) => {
    ...
    const handleChange = ({ file, fileList, event }: any) => {
      if (file.status) {
        ...
        if (props.onChange) {
          props.onChange(newValue as ImageFile[], newFile);
        }
      }
    };
    ...
    const deleteImage = (file: ImageFile, index: number) => {
      ...
      props.onChange?.(images.value as ImageFile[], file);
      emit('update:value', images.value);
    };

    return () => {
      return (
        <div class={'tqu-ant-image-view'}>
